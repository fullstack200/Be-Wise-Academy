...
class FeedbackForm(forms.ModelForm):
    ...
    captcha = ReCaptchaField(widget=ReCaptchaV2Checkbox())  # Add reCAPTCHA
    ...
    def clean_studentGrade(self):
        ...
        if grade < 7 or grade > 10:
            raise forms.ValidationError('Student Grade must be between 7 and 10.')
        return grade

    def clean_noOfStars(self):
        stars = self.cleaned_data.get('noOfStars')
        if stars < 1 or stars > 5:
            raise forms.ValidationError('Ratings must be between 1 and 5.')
        return stars
